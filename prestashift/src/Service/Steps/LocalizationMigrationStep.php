...
class LocalizationMigrationStep
{
    ...
    private function migrateCountries() {
        ...
        foreach ($rows as $row) {
             ...
             // Lang
             $langs = \PrestaShift\Service\LanguageMapper::expand($this->db_connection->query("SELECT * FROM `{$this->prefix}country_lang` WHERE id_country = " . $row['id_country'])->fetchAll(PDO::FETCH_ASSOC));
             foreach ($langs as $l) {
    ...
    private function migrateCurrencies() {
        ...
        foreach ($rows as $row) {
             ...
             // Lang
             $langs = \PrestaShift\Service\LanguageMapper::expand($this->db_connection->query("SELECT * FROM `{$this->prefix}currency_lang` WHERE id_currency = " . $row['id_currency'])->fetchAll(PDO::FETCH_ASSOC));
             foreach ($langs as $l) {
