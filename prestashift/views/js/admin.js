...
var PrestaShift = {
    ...
    checkConnection: function (e) {
        ...
        if (method === 'bridge') {
            ...
            if (!urlVal || !tokenVal) {
                $.growl.error({ title: PrestaShift.t('error'), message: PrestaShift.t('fill_all_fields') });
                return;
            }
        } else {
            ...
            if (!host || !dbname || !user) {
                $.growl.error({ title: PrestaShift.t('error'), message: PrestaShift.t('fill_db_fields') });
                return;
            }
        }

        var $btn = $('#btn-check-connection');
        $btn.prop('disabled', true).html('<i class="icon-refresh icon-spin"></i> ' + PrestaShift.t('testing'));
        ...
        $.ajax({
            ...
            success: function (response) {
                $btn.prop('disabled', false).html(PrestaShift.t('test_connection_btn')); // Restore Text
                ...
                if (response.success) {
                    ...
                    $.growl.notice({ title: PrestaShift.t('success'), message: msg });
                    ...
                    $.growl.error({ title: PrestaShift.t('error'), message: response.message });
                }
            },
            error: function (jqXHR, textStatus, errorThrown) {
                $btn.prop('disabled', false).html(PrestaShift.t('test_connection_btn'));
                $.growl.error({ title: PrestaShift.t('connection_error'), message: textStatus + ': ' + errorThrown });
    ...
    startMigration: function () {
        ...
        PrestaShift.log(PrestaShift.t('preflight_running'));
        $.ajax({
            url: controller_url,
            type: 'POST',
            ...
            success: function (response) {
                ...
                PrestaShift.log(PrestaShift.t('starting_migration'));
                ...
                if (response.success) {
                    ...
                    if (response.next_batch) {
                        ...
                        if (PrestaShift.isPaused) {
                            ...
                            PrestaShift.log(PrestaShift.t('migration_paused'));
                            $('#btn-pause-migration')
                        ...
                        PrestaShift.log(PrestaShift.t('transfer_complete'));
                        $('#current-action').text(PrestaShift.t('post_tasks_running'));
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success && response.has_state) {
                    ...
                    var taskName = s.current_task || PrestaShift.t('unknown');
                    ...
                    $('#btn-clear-session').on('click', function (e) {
                        e.preventDefault();
                        if (confirm(PrestaShift.t('confirm_clear_session'))) {
